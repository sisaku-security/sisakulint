...
- **Piped into a known stdin-data consumer** — when an `echo`/`printf` of a secret is piped into a command that reads stdin as *input data* and never echoes it to stdout, the value cannot reach the log. The canonical example is Sparkle's appcast signing idiom: `printf '%s' "$SPARKLE_PRIVATE_KEY" | generate_appcast --ed-key-file - …` — `generate_appcast` reads the EdDSA private key from stdin (documented in its `--help`) and only uses it for signing; the key is never written to stdout. Pipeline stages that transform stdin to stdout (`base64`, `tr`, `sed`, `jq`, `tee`, …) or that emit decrypted/derived output (`gpg --decrypt`, `openssl enc -d`) are *not* treated as consumers and remain flagged.
