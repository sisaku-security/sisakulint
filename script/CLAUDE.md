...
- The roughly 150 fixtures in script/actions/ are not wired to automated tests. The only Go test that reads real files is pkg/core/cross_file_taint_integration_test.go, and it hardcodes filenames, so renaming those YAMLs breaks the test. Verify fixtures manually with sisakulint script/actions/<file>.yaml.
