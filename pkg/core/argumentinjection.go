...
func (rule *ArgumentInjectionRule) VisitJobPre(node *ast.Job) error {
	...
	for _, s := range node.Steps {
		...
		// Process each line to find command arguments with untrusted input
		...
		for lineIdx, line := range lines {
			...
			// Check if this line contains untrusted expressions as arguments
			for _, expr := range exprs {
				...
				// Check if the expression is used as a command argument (not in quotes safely)
