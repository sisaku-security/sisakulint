...
func TestDeprecatedNodeRuntimeKnownActions(t *testing.T) {
	tests := []struct {
		...
		{"setup-dotnet v4 is node20", "actions/setup-dotnet@v4", "", 1, 1},
		{"setup-dotnet v5 is node24", "actions/setup-dotnet@v5", "", 0, 0},
		{"setup-java v4 is node20", "actions/setup-java@v4", "", 1, 1},
		{"setup-java v5 is node24", "actions/setup-java@v5", "", 0, 0},
		{"stale v9 is node20", "actions/stale@v9", "", 1, 1},
		{"stale v10 is node24", "actions/stale@v10", "", 0, 0},
		{"configure-pages v5 is node20", "actions/configure-pages@v5", "", 1, 1},
		{"configure-pages v6 is node24", "actions/configure-pages@v6", "", 0, 0},
		{"deploy-pages v4 is node20", "actions/deploy-pages@v4", "", 1, 1},
		{"deploy-pages v5 is node24", "actions/deploy-pages@v5", "", 0, 0},
