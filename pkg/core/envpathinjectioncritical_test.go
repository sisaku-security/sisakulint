...
echo "${{ github.event.pull_request.body }}"`,
		...
		{
			name:        "subshell exec append does not affect parent output",
			trigger:     "pull_request_target",
			runScript:   `(exec >> "$GITHUB_PATH"); echo "${{ github.event.pull_request.body }}"`,
			wantErrors:  0,
			description: "Should not let redirect-only exec in a subshell make parent siblings write GITHUB_PATH",
		},
		{
			name:        "function exec append definition does not affect parent output",
			trigger:     "pull_request_target",
			runScript:   `f() { exec >> "$GITHUB_PATH"; }; echo "${{ github.event.pull_request.body }}"`,
			wantErrors:  0,
			description: "Should not let redirect-only exec in a function body make parent siblings write GITHUB_PATH",
		},
		{
			name:        "command substitution exec append does not affect parent output",
			trigger:     "pull_request_target",
			runScript:   `echo "$(exec >> "$GITHUB_PATH")"; echo "${{ github.event.pull_request.body }}"`,
			wantErrors:  0,
			description: "Should not let redirect-only exec in command substitution make parent siblings write GITHUB_PATH",
		},
		{
			name:        "pipeline exec append does not affect parent output",
			trigger:     "pull_request_target",
			runScript:   `exec >> "$GITHUB_PATH" | cat; echo "${{ github.event.pull_request.body }}"`,
			wantErrors:  0,
			description: "Should not let redirect-only exec in a pipeline element make parent siblings write GITHUB_PATH",
		},
		{
			name:        "background exec append does not affect parent output",
			trigger:     "pull_request_target",
			runScript:   `exec >> "$GITHUB_PATH" & echo "${{ github.event.pull_request.body }}"`,
			wantErrors:  0,
			description: "Should not let redirect-only exec in a background job make parent siblings write GITHUB_PATH",
		},
		{
			name:        "brace group exec append affects parent output",
			trigger:     "pull_request_target",
			runScript:   `{ exec >> "$GITHUB_PATH"; }; echo "${{ github.event.pull_request.body }}"`,
			wantErrors:  1,
			description: "Should let redirect-only exec in a brace group affect current-shell siblings",
		},
		{
			name:    "parent exec append does not make uncalled function definition output",
			trigger: "pull_request_target",
			runScript: `exec >> "$GITHUB_PATH"
f() { echo "${{ github.event.pull_request.body }}"; }`,
			wantErrors:  0,
...
echo "$(realpath "$PR_BODY")"`,
		...
		{
			name: "subshell exec append rewrites only subshell output",
			run:  `(exec >> "$GITHUB_PATH"; echo "${{ github.event.pull_request.body }}"); echo "$PR_BODY"`,
			want: `(exec >> "$GITHUB_PATH"; echo "$(realpath "$PR_BODY")"); echo "$PR_BODY"`,
		},
		{
			name: "function exec append rewrites only function body output",
			run:  `f() { exec >> "$GITHUB_PATH"; echo "${{ github.event.pull_request.body }}"; }; echo "$PR_BODY"`,
			want: `f() { exec >> "$GITHUB_PATH"; echo "$(realpath "$PR_BODY")"; }; echo "$PR_BODY"`,
		},
		{
			name: "command substitution exec append rewrites only substitution output",
			run:  `echo "$(exec >> "$GITHUB_PATH"; echo "${{ github.event.pull_request.body }}")"; echo "$PR_BODY"`,
			want: `echo "$(exec >> "$GITHUB_PATH"; echo "$(realpath "$PR_BODY")")"; echo "$PR_BODY"`,
		},
		{
...
// TestEnvPathInjectionCriticalPushesSinkRecord verifies that a detected PATH
// injection finding in a privileged-trigger workflow is also pushed to the
...
func TestEnvPathInjectionCriticalPushesSinkRecord(t *testing.T) {
	collector := chain.NewSinkCollector()
	rule := EnvPathInjectionCriticalRuleWithCollector(collector)
