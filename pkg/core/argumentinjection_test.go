...
func TestArgumentInjection_ExtractCommandName(t *testing.T) {
	rule := ArgumentInjectionCriticalRule()
	...
	tests := []struct {
		...
		{
			line:            "git diff origin/main",
			expectedCommand: "git",
			description:     "Should extract git command",
		},
		{
			line:            "curl -s https://example.com",
			expectedCommand: "curl",
			description:     "Should extract curl command",
		},
		{
			line:            "sudo docker run myimage",
			expectedCommand: "docker",
			description:     "Should extract command after sudo",
		},
		{
			line:            "  npm install express",
			expectedCommand: "npm",
		...
		},
		{
			line:            "echo hello",
			expectedCommand: "",
			description:     "Should return empty for non-dangerous command",
		},
		{
			line:            "unknown-command arg",
			expectedCommand: "",
			description:     "Should return empty for unknown command",
		},
	}

	for _, tt := range tests {
		t.Run(tt.description, func(t *testing.T) {
			result := rule.extractCommandName(tt.line)
			if result != tt.expectedCommand {
				t.Errorf("extractCommandName(%q) = %q, want %q",
					tt.line, result, tt.expectedCommand)
			}
		})
	}
}

func TestArgumentInjection_HasEndOfOptionsMarker(t *testing.T) {
	rule := ArgumentInjectionCriticalRule()
	...
	tests := []struct {
		...
		{
			line:     "git diff -- ${{ github.event.pull_request.head.ref }}",
			exprRaw:  "github.event.pull_request.head.ref",
			expected: true,
			desc:     "Should detect -- marker",
		},
		{
			line:     "git diff ${{ github.event.pull_request.head.ref }}",
			exprRaw:  "github.event.pull_request.head.ref",
			expected: false,
			desc:     "Should not detect without -- marker",
		},
		{
			line:     "git diff --stat ${{ github.event.pull_request.head.ref }}",
			exprRaw:  "github.event.pull_request.head.ref",
			expected: false,
			desc:     "Should not confuse --stat with -- marker",
		},
		{
			line:     "git log -- ${{ github.head_ref }}",
			exprRaw:  "github.head_ref",
			expected: true,
			desc:     "Should detect -- marker with different expr",
		},
	}

	for _, tt := range tests {
		t.Run(tt.desc, func(t *testing.T) {
			result := rule.hasEndOfOptionsMarker(tt.line, tt.exprRaw)
			if result != tt.expected {
				t.Errorf("hasEndOfOptionsMarker(%q, %q) = %v, want %v",
					tt.line, tt.exprRaw, result, tt.expected)
			}
		})
	}
}
