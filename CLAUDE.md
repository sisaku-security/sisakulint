...
- **Taint analysis** (#446): `pkg/shell/taint.go` provides shared, state-less primitives reused by `TaintTracker` (code-injection chain) and `SecretInLogRule`:
  ...
  - `shell.WordLitPrefix(word)` — concatenates leading `Lit` parts (including `Lit` nodes inside top-level `DblQuoted`); covers command-name detection, `-v` flag matching, and `NAME=` prefix scanning (#462; replaced the strict-subset `pkg/core::firstWordLiteral`)
