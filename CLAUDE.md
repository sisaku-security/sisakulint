...
- **DependabotEcosystemRule** - Detects package ecosystems (npm/gomod/pip/cargo/bundler/composer/maven/gradle/sbt) inferred from root-level lockfiles and workflow setup actions that are missing from the dependabot configuration (local-scan only, diagnose-only; github-actions handled by DependabotGitHubActionsRule)
