...
- **SecretExfiltrationRule** - Detects secret exfiltration via network commands (curl, wget, nc, etc.). Supports an `secret-exfiltration.allowed-hosts` allowlist in `.github/sisakulint.yaml` (exact host or `*.suffix` wildcard, case-insensitive) plus a per-workflow comment override `# sisakulint:secret-exfiltration.allowed-hosts: a.example.com, *.b.example.com`. Unused allowlist entries are surfaced as "dead allow" warnings so the list does not silently rot. The allowlist applies only to this rule; `secret-in-log` is intentionally unaffected in Phase 1 (#473)
